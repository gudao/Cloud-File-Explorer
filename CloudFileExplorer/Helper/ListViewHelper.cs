...
namespace CloudFileExplorer.Helper
{
    ...
    public class ListViewColumnSorter : System.Collections.IComparer
    {
        ...
        public int Compare(object x, object y)
        {
            ...
            // 比较
            if (IsSize(xText) && IsSize(yText))
            {
                compareResult = CompareLong(RecoverSize(xText), RecoverSize(yText));
            }
        ...
        public bool IsSize(string size)
        {
            return System.Text.RegularExpressions.Regex.Match(size, @"\d+[K|M|G|T]B", System.Text.RegularExpressions.RegexOptions.IgnoreCase).Success;
        }
        ...
        private double RecoverSize(string sSize)
        {
            long tSize = (long)1024 * 1024 * 1024 * 1024;
            long gSize = 1024 * 1024 * 1024;
            long mSize = 1024 * 1024;

            string intx = sSize.Substring(0, sSize.Length - 2);
            if (sSize.Contains("K"))
                return double.Parse(intx) * 1024;
            else if (sSize.Contains("M"))
                return double.Parse(intx) * mSize;
            else if (sSize.Contains("G"))
                return double.Parse(intx) * gSize;
            else if (sSize.Contains("T"))
                return double.Parse(intx) * tSize;
            else
                return double.Parse(intx);
        }
        private string CoverSize(double fileSize)
        {
            string result = "";
            long tSize = (long)1024 * 1024 * 1024 * 1024;
            long gSize = 1024 * 1024 * 1024;
            ...
            if (fileSize >= tSize)
            {
                result = (fileSize / tSize).ToString("f2") + "TB";
            }
            else if (fileSize >= gSize)
            {
                result = (fileSize / gSize).ToString("f2") + "GB";
            }
            else if (fileSize >= mSize)
            {
                result = (fileSize / mSize).ToString("f2") + "MB";
            }
            else if (fileSize > 1024)
            {
                result = (fileSize / 1024).ToString("f2") + "KB";
            }
            else
            {
                result = fileSize.ToString("f2") + "B";
            }

            return result;
        }

        private int CompareLong(double x, double y)
        {
            if (x > y)
            {
                return 1;
